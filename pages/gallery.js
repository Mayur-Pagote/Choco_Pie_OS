  ...
  <div class="gallery-lightbox" id="gallery-lightbox" role="dialog" aria-modal="true" aria-label="Image viewer">
    ...
    <div class="gallery-lightbox-panel" id="gallery-lb-panel">
      ...
      <div class="gallery-lb-img-wrap">
        <img class="gallery-lb-img" id="gallery-lb-img" src="" alt="" loading="lazy" />
  ...
  grid.innerHTML = items.map((item, idx) => `
    <div class="gallery-card" data-id="${item.id}" data-idx="${idx}" tabindex="0" role="button" aria-label="View ${item.title}">
      <div class="gallery-card-img-wrap">
        <img class="gallery-card-img" src="${item.src}" alt="${item.title}" loading="lazy" onerror="this.parentElement.classList.add('gallery-img-error')" />
...
function bindGalleryEvents() {
  // Filter buttons
  document.getElementById('gallery-filter-bar')?.querySelectorAll('.gallery-filter-btn').forEach(btn => {
...
function bindGridCardEvents() {
  document.querySelectorAll('.gallery-card').forEach(card => {
    ...
    card.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') openLightbox(parseInt(card.dataset.idx)); });
  });
}
